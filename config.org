...
#+end_src
Auto load =bibliography-mode= only when in bibliography-path
#+begin_src emacs-lisp
    (defun strange-loop/bibliography-hook ()
      "Hook to turn on bibliography-mode only when in bibliography-path"
      (cond ((string= (file-truename bibliography-path) buffer-file-name)
             (bibliography-mode t))))

    (add-hook 'org-mode-hook 'strange-loop/bibliography-hook)
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (setq strange-loop/org-agenda-files (list org-index-file "/home/anirudh/test.org"))

...
I want to open my /annotated bibliography/ using =C-c n o=
#+begin_src emacs-lisp
  (defun strange-loop/open-bib ()
    "Open the annotated bibliography."
    (interactive)
    (find-file (org-file-path "papers.org"))
    (end-of-buffer))
  (global-set-key (kbd "C-c n o") 'strange-loop/open-bib)
#+end_src
...
#+begin_src emacs-lisp
  (use-package helm-bibtex
    ...
    (("C-c n b" . helm-bibtex)))
#+end_src
Use =helm-org-ql= to search my /annotated bibliography/
#+begin_src emacs-lisp
  (use-package helm-org-ql
    ...
    (("C-c n s" . helm-org-ql)))
#+end_src
Adding new entries requires a bit of work. Assume that I've downloaded the required the pdf and have obtained the bibtex
#+begin_src emacs-lisp
  (defun strange-loop/add-bib-entry ()
    "Create a new entry in my annotated bibliography"
    (interactive)
    (let ((input
      ...
      (message
       (shell-command-to-string (format "python3 /home/anirudh/.emacs.d/gen-bib-org.py \"%s\"" input)))))

  (global-set-key (kbd "C-c n a") 'strange-loop/add-bib-entry)
#+end_src
*** RSS
Use =elfeed=
#+begin_src emacs-lisp
